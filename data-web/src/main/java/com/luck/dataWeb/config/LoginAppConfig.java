...
@Component
public class LoginAppConfig implements HandlerInterceptor {
    private Logger logger = LoggerFactory.getLogger(SysUserService.class);
    @Autowired
    TokenManagerApp tokenManagerApp;
    @Autowired
...
//                logger.info("当前请求路径是:{}", requestURI);
                ...
                if (StringUtils.isNotEmpty(token)) {
                    TokenApp tokenApp = tokenManagerApp.getTokenUserFromToken(token);
                    String openId = tokenApp.getOpenId();
//                    //保存全局用户信息
//                    accountService.setCurrentUserId(request, openId);
                }
                //后台登录检测
    ...
    private void checkAdminWebTokenAndPerm(HttpServletRequest request) throws Exception {
        ...
        String openId = tokenApp.getOpenId();
        // 更新redis token失效时间
        tokenManagerApp.refreshToken(tokenApp);
//        //保存全局用户信息
//        accountService.setCurrentUserId(request, openId);
    }
}
